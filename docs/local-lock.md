...
egress canary cannot connect, and runs the Rust workspace tests offline. W2's
daemon has no network-capable dependency or request path and reports
`local_lock=true` from health. This is a harness-level proof, not yet the final
desktop enforcement boundary: the broker, model registry, shell, and complete
offline import-to-export scenario land in later phases.
