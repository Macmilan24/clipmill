...
def verify_artifact(artifact_root: Path, artifact_id: str) -> VerifiedArtifact:
    """Verify an artifact under a store root and return what it contains.

    ``artifact_root`` is the store directory the daemon hands a worker on its
    lease.
    """
    ...
    object_directory = Path(artifact_root) / "objects" / "sha256" / digest[:2] / digest
...
def open_artifact(artifact_root: Path, artifact_id: str) -> VerifiedArtifact:
    """Verify an artifact and return a handle to its payload directory."""

    return verify_artifact(artifact_root, artifact_id)


def artifact_file(artifact: VerifiedArtifact, relative: str) -> Path:
    """Resolve one verified payload file inside an artifact.

    The path must be one the manifest declared. Resolving anything else would
    read a file whose digest nobody checked.
    """

    declared = {record["path"] for record in artifact.manifest.get("files", [])}
    if relative not in declared:
        raise ArtifactVerificationError(f"{relative!r} is not a declared payload of this artifact")
    return artifact.object_directory.joinpath(*PurePosixPath(relative).parts)
